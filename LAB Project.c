...
void UpdateLedState(uint32_t temp_F, uint32_t light_reading, uint32_t moist_reading)
{
    ...
    if (temp_F > 660 && light_reading > 800 && moist_reading > 300)
    {
        ...
        if (current_season != SUMMER) {
            if (!first_run) {
                ...
                BzTone(5000, 100); // Play a high pitch sound for 100 ms
            }
    ...
    else if (temp_F <= 670 && light_reading <= 800 && moist_reading <= 300)
    {
        ...
        if (current_season != FALL) {
            if (!first_run) {
                uprintf("%s\n\r","ITS FALL TIME MY BOI\n");
                BzTone(500, 100); // Play a low pitch sound for 100 ms
            }
            current_season = FALL;
        }
    }
